...
static void update_irq(NRF52RNGState *s)
{
...
static uint64_t rng_read(void *opaque, hwaddr offset, unsigned int size)
{
    ...
    case NRF52_RNG_EVENT_VALRDY:
        r = s->event_valrdy;
        break;
    case NRF52_RNG_REG_SHORTS:
        r = s->shortcut_stop_on_valrdy;
        break;
    case NRF52_RNG_REG_INTEN:
    case NRF52_RNG_REG_INTENSET:
    case NRF52_RNG_REG_INTENCLR:
        r = s->interrupt_enabled;
        break;
    case NRF52_RNG_REG_CONFIG:
        r = s->filter_enabled;
        break;
    case NRF52_RNG_REG_VALUE:
        r = s->value;
        break;
...
static int64_t calc_next_timeout(NRF52RNGState *s)
{
...
static void rng_update_timer(NRF52RNGState *s)
{
    if (s->active) {
                       ...
                       uint64_t value, unsigned int size)
{
    ...
    case NRF52_RNG_REG_SHORTS:
        s->shortcut_stop_on_valrdy =
                (value & BIT(NRF52_RNG_REG_SHORTS_VALRDY_STOP)) ? 1 : 0;
        break;
    case NRF52_RNG_REG_INTEN:
        s->interrupt_enabled =
                (value & BIT(NRF52_RNG_REG_INTEN_VALRDY)) ? 1 : 0;
        break;
    case NRF52_RNG_REG_INTENSET:
        if (value & BIT(NRF52_RNG_REG_INTEN_VALRDY)) {
    ...
    case NRF52_RNG_REG_CONFIG:
        s->filter_enabled =
                      (value & BIT(NRF52_RNG_REG_CONFIG_DECEN)) ? 1 : 0;
        break;
...
static const VMStateDescription vmstate_rng = {
    ...
    .fields = (const VMStateField[]) {
        VMSTATE_UINT32(active, NRF52RNGState),
        VMSTATE_UINT32(event_valrdy, NRF52RNGState),
        VMSTATE_UINT32(shortcut_stop_on_valrdy, NRF52RNGState),
        VMSTATE_UINT32(interrupt_enabled, NRF52RNGState),
        VMSTATE_UINT32(filter_enabled, NRF52RNGState),
        VMSTATE_END_OF_LIST()
    }
};
